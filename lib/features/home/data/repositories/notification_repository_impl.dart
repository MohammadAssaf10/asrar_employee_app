    ...
    if (await networkInfo.isConnected) {
      try {
        ...
        if (response.statusCode == 200) {
          await _firestore
              .collection(FireBaseConstants.employeeNotifications)
    ...
    if (await networkInfo.isConnected) {
      try {
        ...
        final notificationDoc =
            await _firestore.collection(FireBaseConstants.employeeNotifications).get();
        for (var notification in notificationDoc.docs) {
          if (notification['userID'] == employeeID) {
