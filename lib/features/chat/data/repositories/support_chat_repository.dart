...
class SupportChatRepository extends ChatRepository {
  ...
  _firestore.collection(FireBaseConstants.supportChat).doc(employee.employeeID);
  ...
  Future<Either<Failure, String>> uploadImage(XFile image) async {
    try {
      var file = await uploadFile(
          '${FireBaseConstants.supportChat}/${employee.employeeID}/images/${image.name}', image);
      print(file.url);
  ...
  Future<Either<Failure, String>> uploadVoice(XFile voice)  async{
    try {
      var file = await uploadFile(
          '${FireBaseConstants.supportChat}/${employee.employeeID}/voices/${voice.name}', voice);
      print(file.url);
      return Right(file.url);
    } catch (e) {
      return Left(ExceptionHandler.handle(e).failure);
